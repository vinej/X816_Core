...
// jyv 2026-07-07: VERA FX address/data engine, ported from the official
// X16Community/vera-module v47.0.2 addr_data.v (FX additions by JeffreyH).
// This module owns ADDR0/ADDR1, DATA0/DATA1, the FX helpers (line draw,
// polygon filler, affine), the 32-bit cache and the multiplier/accumulator.
// Port changes vs upstream: Lattice synthesis pragmas stripped; mult_accum
// is the generic Cyclone V version (see mult_accum.v).
module addr_data(
    input  wire        reset,
    input  wire        clk,
    ...
    localparam
        ...
        ADDR0_INCR_0       = 2'b10,   // ADDR0 should be incremented with the increment of ADDR0

        ADDR1_UNTOUCHED    = 3'b000,  // ADDR1 is untouched and should stay the same
        ADDR1_INCR_1       = 3'b001,  // ADDR1 should be incremented with the increment of ADDR1
        ADDR1_INCR_1_AND_0 = 3'b010,  // ADDR1 should be incremented with both the increment of ADDR1 and the increment of ADDR0
    ...
    // The bit "pixel increment times 32" means that the pixel increment should be multiplied by 32,
    // this effectively changes the increment to be a 11.4 fixed point number (instead of 6.9)
    reg        fx_pixel_incr_x_times_32_r,    fx_pixel_incr_x_times_32_next;
    reg        fx_pixel_incr_y_times_32_r,    fx_pixel_incr_y_times_32_next;

    // Pixel increments are fixed point numbers with a 6-bit integer part and a 9-bit fractional part (6.9)
    ...
    always @* begin
        ...
        fx_vram_addr_0_needs_to_be_changed = ADDR0_UNTOUCHED;
        fx_vram_addr_1_needs_to_be_changed = ADDR1_UNTOUCHED;
        ...
        // In polygon filler mode (when not in poke mode) we increment the pixel positions when reading from DATA1
        // We also calculate the new ADDR1 after updating the pixel positions
        ...
        if (do_write && access_addr == 5'h0C && dc_select == 6'd2) begin
        ...
        if (do_write && (access_addr == 5'h03 || access_addr == 5'h04)) begin
            ...
            if(fx_2bit_poke_mode_r && fx_addr1_mode_r == MODE_POLY_FILL && access_addr == 5'h04) begin
                ...
                // ib_one_byte_cache_cycling_next = 0; // We temporarily turn off the use of cache byte cycling
                ...
                // Only when writing to *main* VRAM do we allow multibyte cache writes or transparency or nibble writes
        ...
        if (do_write && access_addr == 5'h09 && dc_select == 6'd2) begin
        ...
        if (do_write && access_addr == 5'h09 && dc_select == 6'd3) begin
            fx_pixel_incr_x_next[7:0] = write_data;
        end
        if (do_write && access_addr == 5'h0A && dc_select == 6'd3 && fx_addr1_mode_r == MODE_LINE_DRAW) begin
            // In line draw mode we also reset the overflow bit
            fx_pixel_pos_x_next[9] = 1'b0;
        end
        if (do_write && access_addr == 5'h0A && dc_select == 6'd3) begin
        ...
        if (do_write && access_addr == 5'h0B && dc_select == 6'd3) begin
            fx_pixel_incr_y_next[7:0] = write_data;
        end
        if (do_write && access_addr == 5'h0C && dc_select == 6'd3) begin
        ...
        if (do_write && access_addr == 5'h09 && dc_select == 6'd4) begin
            fx_pixel_pos_x_next[16:9] = write_data;
            fx_calculate_addr1_based_on_position_next = 1;
        end
        if (do_write && access_addr == 5'h0A && dc_select == 6'd4) begin
            fx_pixel_pos_x_next[19:17] = write_data[2:0];
            fx_pixel_pos_x_next[0] = write_data[7];
            fx_calculate_addr1_based_on_position_next = 1;
        end
        if (do_write && access_addr == 5'h0B && dc_select == 6'd4) begin
            fx_pixel_pos_y_next[16:9] = write_data;
            fx_calculate_addr1_based_on_position_next = 1;
        end
        if (do_write && access_addr == 5'h0C && dc_select == 6'd4) begin
            fx_pixel_pos_y_next[19:17] = write_data[2:0];
            fx_calculate_addr1_based_on_position_next = 1;
            fx_pixel_pos_y_next[0] = write_data[7];
        end

        if (do_write && access_addr == 5'h09 && dc_select == 6'd5) begin
            fx_pixel_pos_x_next[8:1] = write_data;
        end
        if (do_write && access_addr == 5'h0A && dc_select == 6'd5) begin
            fx_pixel_pos_y_next[8:1] = write_data;
        end
        // 5'h0B / 5'h0C at dc_select 5 are read-only (fill length registers)


        if (do_read && access_addr == 5'h09 && dc_select == 6'd6) begin
            fx_reset_accum_next = 1;
        end
        if (do_read && access_addr == 5'h0A && dc_select == 6'd6) begin
            fx_accumulate_next = 1;
        end

        // Direct access to the cache32
        if (do_write && access_addr == 5'h09 && dc_select == 6'd6) begin
            ib_cache32_next[7:0] = write_data;
        end
        if (do_write && access_addr == 5'h0A && dc_select == 6'd6) begin
            ib_cache32_next[15:8] = write_data;
        end
        if (do_write && access_addr == 5'h0B && dc_select == 6'd6) begin
            ib_cache32_next[23:16] = write_data;
        end
        if (do_write && access_addr == 5'h0C && dc_select == 6'd6) begin
        ...
        case (fx_map_size_r)
            2'b00: begin   // 2x2
                fx_tile_position_repeat = {12'b0, fx_pixel_position_in_map_y[3], fx_pixel_position_in_map_x[3]};
                if (fx_pixel_position_in_map_y[10:4] == 0 && fx_pixel_position_in_map_x[10:4] == 0)
                    fx_position_is_outside_map = 0;
            end
            2'b01: begin   // 8x8
                fx_tile_position_repeat = {8'b0, fx_pixel_position_in_map_y[5:3], fx_pixel_position_in_map_x[5:3]};
                if (fx_pixel_position_in_map_y[10:6] == 0 && fx_pixel_position_in_map_x[10:6] == 0)
                    fx_position_is_outside_map = 0;
            end
            2'b10: begin   // 32x32
                fx_tile_position_repeat = {4'b0, fx_pixel_position_in_map_y[7:3], fx_pixel_position_in_map_x[7:3]};
        ...
        if (fx_4bit_mode_r) begin
            vram_addr_1_tiledata_using_tilemap = {fx_tiledata_base_address_r, 11'b0} + {4'b0, fx_tile_index_looked_up, fx_pixel_position_in_tile_y, fx_pixel_position_in_tile_x[2:1]};
            vram_addr_nib_1_tiledata_using_tilemap = fx_pixel_position_in_tile_x[0];
        end else begin
            vram_addr_1_tiledata_using_tilemap = {fx_tiledata_base_address_r, 11'b0} + {3'b0, fx_tile_index_looked_up, fx_pixel_position_in_tile_y, fx_pixel_position_in_tile_x};
        ...
        case (fx_vram_addr_1_needs_to_be_changed)
            ...
            default: begin  // ADDR1_UNTOUCHED, ADDR1_SET (and the unused value)
                // We leave addr1 unchanged, unless just externally/explicitly set
