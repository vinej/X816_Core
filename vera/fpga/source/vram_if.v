...
// jyv 2026-07-07: VERA FX update -- if0 (CPU port) grew the FX write
// qualifiers (nibble address, 4-bit mode, cache write, transparency,
// one-byte cache cycling) and the 32-bit cache/multiplier data path.
// Ported from X16Community/vera-module v47.0.2 vram_if.v.  Write masking
// is now at nibble granularity (8-bit wrnibblesel) to support 4-bit mode
// and transparent writes; see main_ram.v.
module vram_if(
    input  wire        clk,
    ...
    always @* begin
        ...
        if (if0_cache_write_enabled) begin
            ...
            // In non-cache write mode, we write the byte corresponding to the addr[1:0], unless we are in transparent mode and write a 0
            case (if0_addr[1:0])
